...
export class RequestRouter {
  ...
  async route(request: MCPRequest): Promise<unknown> {
    ...
    try {
      ...
      if (method.startsWith('rpc.')) {
        return await this.handleRPCMethod(_method, params);
      }
      // Handle tool invocations
      if (method.startsWith('tools.')) {
        return await this.handleToolMethod(_method, params);
      }
      // Try to execute as a tool directly
      const _tool = this.toolRegistry.getTool(method);
      if (tool) {
        const _result = await this.toolRegistry.executeTool(_method, params);
  ...
  private discoverMethods(): Record<string, string> {
    const _methods: Record<string, string> = {
  ...
  private describeMethod(params: unknown): unknown {
    ...
    const _builtInMethods: Record<string, unknown> = {
      'rpc.discover': {
  ...
  private async invokeTool(params: unknown): Promise<unknown> {
    ...
    return await this.toolRegistry.executeTool(_tool, input || { /* empty */ });
  }
  /**
  ...
  private describeTool(params: unknown): unknown {
    ...
    const _tool = this.toolRegistry.getTool(toolName);
    if (!tool) {
