...
export class HttpTransport implements ITransport {
  ...
  private setupMiddleware(): void {
    ...
    if (this.config?.corsEnabled) {
      const _origins = this.config.corsOrigins || ['*'];
      this.app.use(cors({
        origin: _origins,
        credentials: true,
  ...
  private setupRoutes(): void {
    ...
    const _consoleDir = join(__dirname, '../../ui/console');
    ...
    this.app.get('/console', (_req, _res) => {
      res.sendFile(join(_consoleDir, 'index.html'));
    });
    ...
    this.app.post('/rpc', async (_req, _res) => {
      await this.handleJsonRpcRequest(_req, res);
    });
    // Handle preflight requests
    this.app.options('*', (_req, _res) => {
      res.status(204).end();
    });
    // 404 handler
    ...
    // Error handler
    this.app.use((err: _any, _req: express._Request, _res: express._Response, _next: express.NextFunction) => {
  ...
  private setupWebSocketHandlers(): void {
    if (!this.wss) return;
    this.wss.on('connection', (ws: _WebSocket, _req) => {
      this.activeWebSockets.add(ws);
      this.logger.info('WebSocket client connected', {
        totalClients: this.activeWebSockets._size,
      });
      ws.on('close', () => {
      ...
      ws.on('message', async (data) => {
        try {
          ...
          if (message.id === undefined) {
            ...
            // Request from client
            const _response = await this.handleRequestMessage(message as MCPRequest);
  ...
  private async handleJsonRpcRequest(_req: _Request, _res: Response): Promise<void> {
    // Check content type
    ...
    if (this.config?.auth?.enabled) {
      const _authResult = await this.validateAuth(req);
      if (!authResult.valid) {
        res.status(401).json({
    ...
    try {
      const _mcpMessage = req.body;
      // Validate JSON-RPC format
      if (!mcpMessage.jsonrpc || mcpMessage.jsonrpc !== '2.0') {
        res.status(400).json({
          jsonrpc: '2.0',
          id: mcpMessage.id || _null,
          error: {
            code: -_32600,
      ...
      if (mcpMessage.id === undefined) {
        ...
        // Handle request
        const _response = await this.handleRequestMessage(mcpMessage as MCPRequest);
        res.json(response);
      }
    } catch (_error) {
  ...
  private async validateAuth(_req: Request): Promise<{ valid: boolean; error?: string }> {
    ...
    const _tokenMatch = auth.match(/^Bearers+(.+)$/i);
    if (!tokenMatch) {
    ...
    if (this.config?.auth?.tokens && this.config.auth.tokens.length > 0) {
      const _isValid = this.config.auth.tokens.includes(token);
      if (!isValid) {
  ...
  async sendNotification(notification: MCPNotification): Promise<void> {
    ...
    const _message = JSON.stringify(notification);
