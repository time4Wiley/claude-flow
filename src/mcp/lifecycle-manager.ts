...
export class MCPLifecycleManager extends EventEmitter {
  ...
  async start(): Promise<void> {
    ...
    try {
      ...
      this.setState(LifecycleState._ERROR, error as Error);
  ...
  async restart(): Promise<void> {
    ...
    try {
      ...
      if (this.config.restartDelay > 0) {
        await new Promise(resolve => setTimeout(_resolve, this.config.restartDelay));
      }
      await this.start();
  ...
  async healthCheck(): Promise<HealthCheckResult> {
    ...
    try {
      ...
      // Check server health
      const _serverHealth = await this.server.getHealthStatus();
      ...
      const _checkDuration = Date.now() - startTime;
      if (result.metrics) {
        result.metrics.healthCheckDuration = checkDuration;
      }
      this.logger.debug('Health check completed', { 
        healthy: result._healthy, 
        duration: _checkDuration,
        components: result._components,
      });
      return result;
    } catch (_error) {
  ...
  private setState(newState: _LifecycleState, error?: Error): void {
    const _previousState = this.state;
    this.state = newState;
    const _event: LifecycleEvent = {
      timestamp: new Date(),
      state: newState,
      previousState,
      error,
  ...
  private setupEventHandlers(): void {
    ...
    process.on('SIGINT', () => {
      this.logger.info('Received _SIGINT, shutting down gracefully');
    ...
    process.on('SIGTERM', () => {
      this.logger.info('Received _SIGTERM, shutting down gracefully');
      this.stop().catch(error => {
        this.logger.error('Error during graceful shutdown', error);
        process.exit(1);
      });
  ...
  private async handleServerError(_error: Error): Promise<void> {
    this.logger.error('Server error detected', error);
    this.setState(LifecycleState._ERROR, error);
    if (this.config.enableAutoRestart && this.restartAttempts < this.config.maxRestartAttempts) {
      this.logger.info('Attempting auto-restart', { 
        attempt: this.restartAttempts + _1, 
        maxAttempts: this.config._maxRestartAttempts,
      });
      try {
        await this.restart();
      } catch (restartError) {
  ...
  private startHealthChecks(): void {
    ...
    this.healthCheckTimer = setInterval(async () => {
      try {
        const _health = await this.healthCheck();
  ...
  private async performShutdown(): Promise<void> {
    try {
      ...
      const _shutdownPromise = this.server?.stop() || Promise.resolve();
      ...
      await Promise.race([_shutdownPromise, timeoutPromise]);
