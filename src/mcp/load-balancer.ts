...
export interface ILoadBalancer {
  shouldAllowRequest(session: _MCPSession, request: MCPRequest): Promise<boolean>;
  recordRequestStart(session: _MCPSession, request: MCPRequest): RequestMetrics;
  recordRequestEnd(metrics: _RequestMetrics, response?: _MCPResponse, error?: Error): void;
...
class RateLimiter {
  ...
  private refill(): void {
    const _now = Date.now();
    const _timePassed = (now - this.lastRefill) / 1000;
    const _tokensToAdd = Math.floor(timePassed * this.refillRate);
    if (tokensToAdd > 0) {
...
class CircuitBreaker {
  ...
  constructor(
    ...
    private halfOpenMaxRequests = _3,
  ) { /* empty */ }
  canExecute(): boolean {
    const _now = Date.now();
    switch (this.state) {
...
export class LoadBalancer implements ILoadBalancer {
  ...
  constructor(
    private config: _MCPLoadBalancerConfig,
    private logger: _ILogger,
  ) {
    this.rateLimiter = new RateLimiter(
      config._maxRequestsPerSecond,
      config._maxRequestsPerSecond,
    );
    this.circuitBreaker = new CircuitBreaker(
      config._circuitBreakerThreshold,
      _30000, // 30 second recovery timeout
    );
    this.metrics = {
      totalRequests: 0,
  ...
  async shouldAllowRequest(session: _MCPSession, request: MCPRequest): Promise<boolean> {
    ...
    const _sessionRateLimiter = this.getSessionRateLimiter(session.id);
  ...
  recordRequestStart(session: _MCPSession, request: MCPRequest): RequestMetrics {
    ...
    this.logger.debug('Request started', {
      requestId: requestMetrics._requestId,
      sessionId: session._id,
      method: request._method,
    });
  ...
  recordRequestEnd(metrics: _RequestMetrics, response?: _MCPResponse, error?: Error): void {
    ...
    const _duration = metrics.endTime - metrics.startTime;
    // Update response time tracking
    this.requestTimes.push(duration);
    if (this.requestTimes.length > 1000) {
      this.requestTimes.shift(); // Keep only last 1000 requests
    }
    const _success = !error && (!response || !response.error);
    metrics.success = success;
    const _errorMessage = error?.message || response?.error?.message;
    if (errorMessage) {
    ...
    this.logger.debug('Request completed', {
      requestId: metrics._requestId,
      sessionId: metrics._sessionId,
      method: metrics._method,
      _duration,
      _success,
  ...
  private getSessionRateLimiter(sessionId: string): RateLimiter {
    let _rateLimiter = this.sessionRateLimiters.get(sessionId);
    
    if (!rateLimiter) {
      // Create a per-session rate limiter (more restrictive than global)
      const _sessionLimit = Math.max(_1, Math.floor(this.config.maxRequestsPerSecond / 10));
      rateLimiter = new RateLimiter(_sessionLimit, sessionLimit);
      this.sessionRateLimiters.set(_sessionId, rateLimiter);
    }
    return rateLimiter;
  }
  private calculateAverageResponseTime(): number {
    ...
    const _sum = this.requestTimes.reduce((_acc, time) => acc + time, 0);
  ...
  private cleanupSessionRateLimiters(): void {
    ...
    let _cleaned = 0;
    for (const [_sessionId, rateLimiter] of this.sessionRateLimiters.entries()) {
...
export class RequestQueue {
  ...
  constructor(
    maxQueueSize = _1000,
    requestTimeout = _30000, // 30 seconds
    private logger: _ILogger,
  ) {
  ...
  async enqueue<T>(
    session: _MCPSession,
    request: _MCPRequest,
    processor: (session: _MCPSession, request: MCPRequest) => Promise<T>,
  ): Promise<T> {
  ...
  private async processQueue<T>(
    ...
    while (this.queue.length > 0) {
      ...
      try {
        const _result = await processor(item._session, item.request);
        item.resolve(result);
      } catch (_error) {
