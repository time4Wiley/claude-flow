...
class FallbackMemoryStore {
  ...
  async store(_key, _value, options = { /* empty */ }) {
    ...
    return this.activeStore.store(_key, _value, options);
  }
  async retrieve(_key, options = { /* empty */ }) {
    await this.initialize();
    return this.activeStore.retrieve(_key, options);
  }
  ...
  async delete(_key, options = { /* empty */ }) {
    await this.initialize();
    return this.activeStore.delete(_key, options);
  }
  async search(_pattern, options = { /* empty */ }) {
    await this.initialize();
    return this.activeStore.search(_pattern, options);
  }
  async cleanup() {
    await this.initialize();
...
export const _memoryStore = new FallbackMemoryStore();
