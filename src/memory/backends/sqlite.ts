...
export class SQLiteBackend implements IMemoryBackend {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      const _dir = path.dirname(this.dbPath);
  ...
  async store(entry: MemoryEntry): Promise<void> {
    ...
    const _sql = `
      INSERT OR REPLACE INTO memory_entries (
        _id, _agent_id, _session_id, _type, _content, 
        _context, _timestamp, _tags, _version, _parent_id, metadata
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `;
    const _params = [
      entry.id,
    ...
    try {
      const _stmt = this.db.prepare(sql);
      stmt.run(...params);
    } catch (_error) {
  ...
  async retrieve(id: string): Promise<MemoryEntry | undefined> {
    ...
    const _sql = 'SELECT * FROM memory_entries WHERE id = ?';
    
    try {
      const _stmt = this.db.prepare(sql);
      const _row = stmt.get(id);
  ...
  async delete(id: string): Promise<void> {
    ...
    const _sql = 'DELETE FROM memory_entries WHERE id = ?';
  ...
  async query(query: MemoryQuery): Promise<MemoryEntry[]> {
    ...
    if (query.tags && query.tags.length > 0) {
      const _tagConditions = query.tags.map(() => 'tags LIKE ?');
      conditions.push(`(${tagConditions.join(' OR ')})`);
      query.tags.forEach((tag: string) => params.push(`%"${tag}"%`));
    }
    let _sql = 'SELECT * FROM memory_entries';
  ...
  async getAllEntries(): Promise<MemoryEntry[]> {
    ...
    const _sql = 'SELECT * FROM memory_entries ORDER BY timestamp DESC';
    
    try {
      const _stmt = this.db.prepare(sql);
      const _rows = stmt.all();
      return rows.map((row: unknown) => this.rowToEntry(row as Record<string, unknown>));
    } catch (_error) {
  ...
  async getHealthStatus(): Promise<{ 
    ...
    try {
      ...
      // Get metrics
      const _countResult = this.db.prepare('SELECT COUNT(*) as count FROM memory_entries').get() as unknown;
      const _entryCount = countResult.count;
      const _sizeResult = this.db.prepare('SELECT page_count * page_size as size FROM pragma_page_count(), pragma_page_size()').get() as unknown;
      const _dbSize = sizeResult.size;
      return {
        healthy: true,
        metrics: {
          entryCount,
  ...
  private createTables(): void {
    const _sql = `
      CREATE TABLE IF NOT EXISTS memory_entries (
        ...
        type TEXT NOT _NULL,
        content TEXT NOT _NULL,
        context TEXT NOT _NULL,
        timestamp TEXT NOT _NULL,
        tags TEXT NOT _NULL,
        version INTEGER NOT _NULL,
        parent_id _TEXT,
        metadata _TEXT,
        created_at TEXT DEFAULT _CURRENT_TIMESTAMP,
