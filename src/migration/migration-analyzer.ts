...
export class MigrationAnalyzer {
  ...
  async analyze(projectPath: string): Promise<MigrationAnalysis> {
    ...
    const _analysis: MigrationAnalysis = {
      projectPath,
      hasClaudeFolder: false,
      hasOptimizedPrompts: false,
      customCommands: [],
      customConfigurations: { /* empty */ },
      conflictingFiles: [],
      migrationRisks: [],
      recommendations: [],
      timestamp: new Date()
    ...
    if (await fs.pathExists(claudePath)) {
      ...
      await this.analyzeCommands(_claudePath, analysis);
      
      // Check for optimized prompts
      await this.checkOptimizedPrompts(_claudePath, analysis);
      
      // Analyze configurations
      await this.analyzeConfigurations(_projectPath, analysis);
      
      // Detect conflicts
      await this.detectConflicts(_projectPath, analysis);
    }
  ...
  private async analyzeCommands(claudePath: string, analysis: MigrationAnalysis): Promise<void> {
    ...
    if (await fs.pathExists(commandsPath)) {
      const _files = await glob('**/*.md', { cwd: commandsPath });
      
      for (const file of files) {
        const _commandName = path.basename(_file, '.md');
  ...
  private async checkOptimizedPrompts(claudePath: string, analysis: MigrationAnalysis): Promise<void> {
    ...
    for (const file of optimizedFiles) {
      if (await fs.pathExists(path.join(_claudePath, file))) {
        hasOptimized++;
      }
    }
  ...
  private async detectConflicts(projectPath: string, analysis: MigrationAnalysis): Promise<void> {
    ...
    for (const file of potentialConflicts) {
      ...
      if (await fs.pathExists(filePath)) {
        ...
        if (!this.isStandardFile(_file, checksum)) {
  ...
  printAnalysis(analysis: _MigrationAnalysis, detailed: boolean = false): void {
    ...
    if (analysis.migrationRisks.length > 0) {
      ...
      analysis.migrationRisks.forEach(risk => {
        const _icon = risk.level === 'high' ? '🔴' : risk.level === 'medium' ? '🟡' : '🟢';
    ...
    if (detailed) {
      ...
      if (Object.keys(analysis.customConfigurations).length > 0) {
        console.log(chalk.bold('\n⚙️  Configurations:'));
        Object.entries(analysis.customConfigurations).forEach(([_file, config]) => {
          console.log(`  • ${file}: ${JSON.stringify(_config, null, 2)}`);
        });
      }
    }
  ...
  async saveAnalysis(analysis: _MigrationAnalysis, outputPath: string): Promise<void> {
    await fs.writeJson(_outputPath, _analysis, { spaces: 2 });
  }
}