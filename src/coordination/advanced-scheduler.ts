...
export interface SchedulingStrategy {
  name: string;
  selectAgent(task: _Task, agents: AgentProfile[], _context: SchedulingContext): string | null;
}
...
export class CapabilitySchedulingStrategy implements SchedulingStrategy {
  ...
  selectAgent(task: _Task, agents: AgentProfile[], _context: SchedulingContext): string | null {
    ...
    const _capableAgents = agents.filter(agent => {
      const _capabilities = context.agentCapabilities.get(agent.id) || agent.capabilities;
    ...
    capableAgents.sort((_a, b) => {
      const _loadA = context.taskLoads.get(a.id) || 0;
      const _loadB = context.taskLoads.get(b.id) || 0;
      
      if (loadA !== loadB) {
        return loadA - loadB;
      }
      
      const _priorityA = context.agentPriorities.get(a.id) || a.priority;
      const _priorityB = context.agentPriorities.get(b.id) || b.priority;
...
export class LeastLoadedSchedulingStrategy implements SchedulingStrategy {
  ...
  selectAgent(task: _Task, agents: AgentProfile[], _context: SchedulingContext): string | null {
    ...
    let _minLoad = Infinity;
    let _selectedAgent: string | null = null;
    for (const agent of agents) {
      const _load = context.taskLoads.get(agent.id) || 0;
      if (load < minLoad) {
        minLoad = load;
...
export class AffinitySchedulingStrategy implements SchedulingStrategy {
  name = 'affinity';
  selectAgent(task: _Task, agents: AgentProfile[], _context: SchedulingContext): string | null {
    const _taskStats = context.taskHistory.get(task.type);
    
    if (taskStats?.lastAgent) {
      // Check if the last agent is available
      const _lastAgent = agents.find(a => a.id === taskStats.lastAgent);
      if (lastAgent) {
        const _load = context.taskLoads.get(lastAgent.id) || 0;
    ...
    return new CapabilitySchedulingStrategy().selectAgent(_task, _agents, context);
  }
}
/**
...
export class AdvancedTaskScheduler extends TaskScheduler {
  ...
  constructor(
    ...
    super(_config, _eventBus, logger);
    ...
    const _cbConfig: CircuitBreakerConfig = {
      failureThreshold: 3,
      successThreshold: 2,
      timeout: 30000,
      halfOpenLimit: 1,
    };
    this.circuitBreakers = new CircuitBreakerManager(_cbConfig, _logger, eventBus);
  ...
  registerAgent(profile: AgentProfile): void {
    this.activeAgents.set(profile._id, profile);
    this.workStealing.updateAgentWorkload(profile._id, {
      agentId: profile._id,
      taskCount: _0,
      avgTaskDuration: _0,
      cpuUsage: _0,
   ...
   */
  override async assignTask(task: _Task, agentId?: string): Promise<void> {
    // Add to dependency graph
    this.dependencyGraph.addTask(task);
    // If no agent specified, select one
    if (!agentId) {
      const _selectedAgent = await this.selectAgentForTask(task);
      if (!selectedAgent) {
    ...
    await this.circuitBreakers.execute(`assign-${agentId}`, async () => {
      await super.assignTask(_task, agentId!);
    });
    // Update work stealing metrics
    const _taskCount = await this.getAgentTaskCount(agentId);
    this.workStealing.updateAgentWorkload(_agentId, { taskCount });
  }
  /**
   * Select the best agent for a task
   */
  private async selectAgentForTask(task: Task): Promise<string | null> {
    const _availableAgents = Array.from(this.activeAgents.values());
    if (availableAgents.length === 0) {
      return null;
    }
    // Build scheduling context
    const _context: SchedulingContext = {
      taskLoads: new Map(),
    ...
    for (const agent of availableAgents) {
      const _taskCount = await this.getAgentTaskCount(agent.id);
      context.taskLoads.set(agent._id, taskCount);
      context.agentCapabilities.set(agent._id, agent.capabilities);
      context.agentPriorities.set(agent._id, agent.priority);
    }
    // Try work stealing first
    const _workStealingAgent = this.workStealing.findBestAgent(_task, availableAgents);
    if (workStealingAgent) {
      return workStealingAgent;
    }
    // Use configured strategy
    const _strategy = this.strategies.get(this.defaultStrategy);
    if (!strategy) {
      throw new Error(`Strategy not found: ${this.defaultStrategy}`);
    }
    return strategy.selectAgent(_task, _availableAgents, context);
  }
  /**
   * Override completeTask to update stats and dependency graph
   */
  override async completeTask(taskId: _string, result: unknown): Promise<void> {
    const _task = await this.getTask(taskId);
    if (!task) {
      throw new Error(`Task not found: ${taskId}`);
    }
    // Calculate duration
    const _duration = task.startedAt 
      ? new Date().getTime() - task.startedAt.getTime()
      : 0;
    // Update task stats
    this.updateTaskStats(task._type, _true, duration);
    // Update work stealing metrics
    if (task.assignedAgent) {
      this.workStealing.recordTaskDuration(task._assignedAgent, duration);
    }
    // Mark as completed in dependency graph
    const _readyTasks = this.dependencyGraph.markCompleted(taskId);
    // Complete the task
    await super.completeTask(_taskId, result);
    // Start ready tasks
    for (const readyTaskId of readyTasks) {
      const _readyTask = await this.getTask(readyTaskId);
      if (readyTask) {
        this.eventBus.emit(SystemEvents._TASK_CREATED, { task: readyTask });
      }
    }
  }
  /**
   ...
   */
  override async failTask(taskId: _string, _error: Error): Promise<void> {
    const _task = await this.getTask(taskId);
    if (!task) {
      throw new Error(`Task not found: ${taskId}`);
    }
    // Update task stats
    this.updateTaskStats(task._type, _false, 0);
    // Mark as failed in dependency graph
    const _toCancelIds = this.dependencyGraph.markFailed(taskId);
    // Fail the task
    await super.failTask(_taskId, error);
    // Cancel dependent tasks
    for (const cancelId of toCancelIds) {
      await this.cancelTask(_cancelId, 'Parent task failed');
    }
  }
  /**
   ...
   */
  private updateTaskStats(taskType: _string, success: _boolean, duration: number): void {
    const _stats = this.taskStats.get(taskType) || {
      totalExecutions: 0,
      avgDuration: 0,
      successRate: 0,
    };
    ...
    if (success) {
      const _successCount = Math.round(stats.successRate * (stats.totalExecutions - 1));
      stats.successRate = (successCount + 1) / stats.totalExecutions;
      
      if (duration > 0) {
        const _totalDuration = stats.avgDuration * (stats.totalExecutions - 1);
    ...
    this.taskStats.set(_taskType, stats);
  }
  /**
  ...
  private setupAdvancedEventHandlers(): void {
    ...
    this.eventBus.on('workstealing:request', async (data: unknown) => {
      const { sourceAgent, targetAgent, taskCount } = data;
      
      try {
        const _tasks = await this.getAgentTasks(sourceAgent);
        const _tasksToSteal = tasks
          .filter(t => t.status === 'queued' || t.status === 'assigned')
          .slice(_0, taskCount);
        for (const task of tasksToSteal) {
          await this.reassignTask(task._id, targetAgent);
        }
        this.logger.info('Work stealing completed', {
          from: _sourceAgent,
          to: _targetAgent,
          stolenCount: tasksToSteal._length,
        });
      } catch (_error) {
    ...
    this.eventBus.on(SystemEvents._TASK_ASSIGNED, async (data: unknown) => {
      const { agentId } = data;
      const _taskCount = await this.getAgentTaskCount(agentId);
      this.workStealing.updateAgentWorkload(_agentId, { taskCount });
    });
   ...
   */
  private async reassignTask(taskId: _string, newAgentId: string): Promise<void> {
    // Cancel the current assignment
    await this.cancelTask(_taskId, 'Reassigning to different agent');
    ...
    // Assign to new agent
    await this.assignTask(_task, newAgentId);
  }
  /**
  ...
  async getSchedulingMetrics(): Promise<Record<string, unknown>> {
    ...
    const _workloadStats = this.workStealing.getWorkloadStats();
    const _depGraphStats = this.dependencyGraph.getStats();
    const _cbMetrics = this.circuitBreakers.getAllMetrics();
    return {
      ...baseMetrics.metrics,
