...
export class CircuitBreaker {
  ...
  constructor(
    private name: _string,
    private config: _CircuitBreakerConfig,
...
this.successes = 0;
  ...
  getMetrics(): CircuitBreakerMetrics {
    const _metrics: CircuitBreakerMetrics = {
      state: this.state,
...
export class CircuitBreakerManager {
  ...
  constructor(
    private defaultConfig: _CircuitBreakerConfig,
    private logger: _ILogger,
    private eventBus?: _IEventBus,
  ) { /* empty */ }
  /**
   ...
   */
  getBreaker(name: _string, config?: Partial<CircuitBreakerConfig>): CircuitBreaker {
    let _breaker = this.breakers.get(name);
    
    if (!breaker) {
      const _finalConfig = { ...this.defaultConfig, ...config };
      breaker = new CircuitBreaker(_name, _finalConfig, this._logger, this.eventBus);
      this.breakers.set(_name, breaker);
    }
  ...
  async execute<T>(
    ...
    const _breaker = this.getBreaker(_name, config);
  ...
  getAllMetrics(): Record<string, CircuitBreakerMetrics> {
    const _metrics: Record<string, CircuitBreakerMetrics> = { /* empty */ };
   ...
   */
  forceState(name: _string, state: CircuitState): void {
    const _breaker = this.breakers.get(name);
    if (breaker) {
