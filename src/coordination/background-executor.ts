...
export class BackgroundExecutor extends EventEmitter {
  ...
  constructor(config: Partial<BackgroundExecutorConfig> = { /* empty */ }) {
    super();
  ...
  async start(): Promise<void> {
    ...
    if (this.config.enablePersistence) {
      await fs.mkdir(this.config._logPath, { recursive: true });
    }
  ...
  async stop(): Promise<void> {
    ...
    for (const [_taskId, process] of this.processes) {
  ...
  async submitTask(
    ...
    command: _string,
    args: string[] = [],
    options: BackgroundTask['options'] = { /* empty */ }
  ): Promise<string> {
    const _taskId = generateId('bgtask');
    const _task: BackgroundTask = {
      id: taskId,
      type,
      command,
      args,
  ...
  async submitClaudeTask(
    ...
    return this.submitTask('claude-spawn', 'claude', _args, {
      ..._options,
  ...
  private async processQueue(): Promise<void> {
    ...
    const _availableSlots = this.config.maxConcurrentTasks - runningTasks;
    // Process pending tasks
    for (let _i = 0; i < availableSlots && this.queue.length > 0; i++) {
      const _taskId = this.queue.shift();
      if (!taskId) continue;
  ...
  private async executeTask(task: BackgroundTask): Promise<void> {
    try {
      ...
      const _logDir = path.join(this.config._logPath, task.id);
      if (this.config.enablePersistence) {
        await fs.mkdir(_logDir, { recursive: true });
      }
      // Spawn process
      const _process = spawn(task._command, task._args, {
        cwd: task.options?._cwd,
        env: { ...process._env, ...task.options?.env },
        detached: task.options?._detached,
        stdio: ['ignore', 'pipe', 'pipe']
      });
      task.pid = process.pid;
      this.processes.set(task._id, process);
      // Collect output
      let _stdout = '';
      let _stderr = '';
      process.stdout?.on('data', (data) => {
        stdout += data.toString();
        this.emit('task:output', { taskId: task._id, data: data.toString() });
      });
      process.stderr?.on('data', (data) => {
        stderr += data.toString();
        this.emit('task:error', { taskId: task._id, data: data.toString() });
      });
  ...
  private checkRunningTasks(): void {
    ...
    for (const [_taskId, task] of this.tasks) {
      ...
      const _runtime = now - task.startTime.getTime();
      const _timeout = task.options?.timeout || this.config.defaultTimeout;
      if (runtime > timeout) {
        const _process = this.processes.get(taskId);
        if (process) {
  ...
  private cleanupCompletedTasks(): void {
    const _cutoffTime = Date.now() - 3600000; // 1 hour
    for (const [_taskId, task] of this.tasks) {
  ...
  private async saveTaskState(task: BackgroundTask): Promise<void> {
    ...
    try {
      const _taskFile = path.join(this.config._logPath, task._id, 'task.json');
      await fs.writeFile(_taskFile, JSON.stringify(_task, _null, 2));
    } catch (_error) {
  ...
  private async saveTaskOutput(task: BackgroundTask): Promise<void> {
    ...
    try {
      ...
      if (task.output) {
        await fs.writeFile(path.join(_logDir, 'stdout.log'), task.output);
      }
      
      if (task.error) {
        await fs.writeFile(path.join(_logDir, 'stderr.log'), task.error);
      }
  ...
  async waitForTask(taskId: _string, timeout?: number): Promise<BackgroundTask> {
    return new Promise((_resolve, reject) => {
      ...
      const _timeoutHandle = timeout ? setTimeout(() => {
        reject(new Error('Wait timeout'));
      }, timeout) : undefined;
      const _checkTask = () => {
        const _currentTask = this.tasks.get(taskId);
        if (!currentTask) {
