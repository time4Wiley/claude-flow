...
class NativeTerminal implements Terminal {
  ...
  async initialize(): Promise<void> {
    try {
      const _shellConfig = this.getShellConfig();
      
      // Start shell process
      this.process = spawn(shellConfig._path, shellConfig._args, {
        stdio: ['pipe', 'pipe', 'pipe'],
        env: {
          ...process._env,
          ...shellConfig._env,
          CLAUDE_FLOW_TERMINAL: 'true',
          CLAUDE_FLOW_TERMINAL_ID: this._id,
        },
      });
  ...
  async executeCommand(command: string): Promise<string> {
    ...
    try {
      ...
      const _markedCommand = this.wrapCommand(command);
      await this.write(markedCommand + '\n');
      // Wait for command to complete
      const _output = await timeout(
        this.commandDeferred._promise,
        _30000,
      ...
      throw new TerminalCommandError('Failed to execute command', { _command, error });
    }
  }
  async write(data: string): Promise<void> {
    ...
    return new Promise((_resolve, reject) => {
      ...
      this.process.stdin.write(_data, (error) => {
        if (_error) {
          reject(error);
        } else {
          resolve();
  ...
  async kill(): Promise<void> {
    ...
    try {
      ...
      this.logger.warn('Error killing native terminal', { id: this._id, error });
    } finally {
  ...
  private setupOutputHandlers(): void {
    ...
    this.process.on('error', (error) => {
      if (this.alive) {
        this.logger.error('Process error', { id: this._id, error });
      }
    });
  }
  private processOutput(text: string): void {
    ...
    if (markerIndex !== -1 && this.commandDeferred) {
      ...
      const _fullOutput = this.errorBuffer ? `${output}\n${this.errorBuffer}` : output;
      this.errorBuffer = '';
  ...
  private notifyListeners(data: string): void {
    this.outputListeners.forEach(listener => {
      try {
        ...
        this.logger.error('Error in output listener', { id: this._id, error });
      }
    });
  }
  private async monitorProcess(): Promise<void> {
    ...
    this.process.on('error', (error) => {
      this.logger.error('Error monitoring process', { id: this._id, error });
      this.alive = false;
  ...
  private async waitForReady(): Promise<void> {
    ...
    const _testCommand = this.shell === 'powershell' 
      ? 'Write-Host "READY"'
      : 'echo "READY"';
...
export class NativeAdapter implements ITerminalAdapter {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      const _testConfig = this.getTestCommand();
      const { spawnSync } = require('child_process');
      const _result = spawnSync(testConfig._cmd, testConfig._args, { stdio: 'ignore' });
      
      if (result.status !== 0) {
        throw new Error('Shell test failed');
      }
    } catch (_error) {
      this.logger.warn(`Shell ${this.shell} not _available, falling back to sh`, { error });
      this.shell = 'sh';
    }
  }
  ...
  async createTerminal(): Promise<Terminal> {
    const _terminal = new NativeTerminal(this._shell, this.logger);
  ...
  private detectShell(): string {
    ...
    if (osplatform === 'win32') {
      ...
      try {
        const { spawnSync } = require('child_process');
        const _result = spawnSync('powershell', ['-Version'], { stdio: 'ignore' });
        if (result.status === 0) {
          return 'powershell';
        }
      } catch {
      ...
      const _shells = ['bash', 'zsh', 'sh'];
      for (const shell of shells) {
        try {
          const { spawnSync } = require('child_process');
          const _result = spawnSync('which', [shell], { stdio: 'ignore' });
          if (result.status === 0) {
            return shell;
          }
        } catch {
