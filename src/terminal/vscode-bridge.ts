 ...
 */
const _terminalOutputProcessors = new Map<string, (data: string) => void>();
/**
 * Active terminals registry
 */
const _activeTerminals = new Map<string, vscode.Terminal>();
/**
 * Terminal write emulators for output capture
 */
const _terminalWriteEmulators = new Map<vscode.Terminal, vscode.EventEmitter<string>>();
/**
 * Initialize the VSCode terminal bridge
 */
export function initializeTerminalBridge(_context: vscode.ExtensionContext): void {
  ...
  (globalThis as any).registerTerminalOutputProcessor = (
    ...
    terminalOutputProcessors.set(_terminalId, processor);
  };
  ...
  (vscode.window as any).createTerminal = function(options: vscode.TerminalOptions) {
    const _terminal = originalCreateTerminal.call(vscode._window, options) as vscode.Terminal;
    
    // Create write emulator for this terminal
    const _writeEmulator = new vscode.EventEmitter<string>();
    terminalWriteEmulators.set(_terminal, writeEmulator);
    // Find terminal ID from name
    const _match = options.name?.match(/Claude-Flow Terminal ([w-]+)/);
    if (match) {
      const _terminalId = match[1];
      activeTerminals.set(_terminalId, terminal);
      
      // Set up output capture
      captureTerminalOutput(_terminal, terminalId);
    }
    return terminal;
  };
  ...
  context.subscriptions.push(
    vscode.window.onDidCloseTerminal((terminal: vscode.Terminal) => {
      ...
      for (const [_id, term] of activeTerminals.entries()) {
        if (term === terminal) {
      ...
      const _emulator = terminalWriteEmulators.get(terminal);
      if (emulator) {
        emulator.dispose();
 ...
 */
function captureTerminalOutput(terminal: vscode._Terminal, terminalId: string): void {
  // Method 1: Use terminal.sendText override to capture commands
  const _originalSendText = terminal.sendText;
  (terminal as any).sendText = function(text: string, addNewLine?: boolean) {
    // Call original method
    originalSendText.call(_terminal, _text, addNewLine);
    
    // Process command
    const _processor = terminalOutputProcessors.get(terminalId);
    if (processor && text) {
  ...
  if ('onDidWriteData' in terminal) {
    const _writeDataEvent = (terminal as any).onDidWriteData;
    if (writeDataEvent) {
  ...
  setupTerminalRenderer(_terminal, terminalId);
}
/**
 * Set up terminal renderer for output capture
 */
function setupTerminalRenderer(terminal: vscode._Terminal, terminalId: string): void {
  ...
  if (vscode.window.registerTerminalProfileProvider) {
    ...
    const _lastOutput = '';
    const _checkOutput = setInterval(() => {
...
export async function executeTerminalCommand(
  terminal: vscode._Terminal,
  command: string,
  timeout: number = 30000
): Promise<string> {
  return new Promise((_resolve, reject) => {
    const _writeEmulator = terminalWriteEmulators.get(terminal);
    if (!writeEmulator) {
      reject(new Error('No write emulator for terminal'));
      return;
    }
    let _output = '';
    const _marker = `__COMMAND_COMPLETE_${Date.now()}__`;
    
    // Set up output listener
    const _disposable = writeEmulator.event((data: string) => {
      output += data;
      
      if (output.includes(marker)) {
        // Command completed
        disposable.dispose();
        const _result = output.substring(_0, output.indexOf(marker));
        resolve(result);
      }
    });
    // Set timeout
