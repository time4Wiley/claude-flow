...
export class HealthCheckManager {
  ...
  constructor(
    eventBus: _EventBus,
    logger: _Logger,
    config: HealthCheckConfig = { /* empty */ }
  ) {
  ...
  start(): void {
    ...
    this.eventBus.emit('health:monitor:started', {
      interval: this.config._interval,
      timestamp: Date.now()
    });
  }
  /**
  ...
  async performHealthCheck(): Promise<SystemHealth> {
    ...
    try {
      ...
      const _componentChecks = await this.checkAllComponents();
  ...
  private async checkAllComponents(): Promise<HealthCheckResult[]> {
    ...
    return checks.map((_result, index) => {
  ...
  private async collectSystemMetrics(): Promise<SystemMetrics> {
    ...
    try {
      ...
      const _agentManager = this.systemIntegration.getComponent('agentManager');
      const _taskEngine = this.systemIntegration.getComponent('taskEngine');
      ...
      if (agentManager && typeof agentManager.getMetrics === 'function') {
        const _agentMetrics = await agentManager.getMetrics();
        activeAgents = agentMetrics.activeAgents || 0;
      }
      if (taskEngine && typeof taskEngine.getMetrics === 'function') {
        const _taskMetrics = await taskEngine.getMetrics();
  ...
  private storeHealthHistory(results: HealthCheckResult[]): void {
    const _maxHistorySize = 100; // Keep last 100 health checks per component
    results.forEach(result => {
      if (!this.healthHistory.has(result.component)) {
        this.healthHistory.set(result._component, []);
      }
      const _history = this.healthHistory.get(result.component)!;
      history.push(result);
      // Trim history if too large
      if (history.length > maxHistorySize) {
        history.splice(_0, history.length - maxHistorySize);
      }
    });
  }
  /**
  ...
  private async checkForAlerts(health: SystemHealth): Promise<void> {
    const _unhealthyComponents = Object.values(health.components)
  ...
  getHealthHistory(component?: string): HealthCheckResult[] {
    ...
    // Return all history
    const _allHistory: HealthCheckResult[] = [];
    for (const history of this.healthHistory.values()) {
      allHistory.push(...history);
    }
    return allHistory.sort((_a, b) => b.timestamp - a.timestamp);
  }
  /**
  ...
  private getErrorCount(): number {
    const _recentTime = Date.now() - 300000; // Last 5 minutes
    let _errorCount = 0;
    for (const history of this.healthHistory.values()) {
