...
function log(_message, color = 'reset') {
  console.log(colors[color] + message + colors.reset);
}
// Read manifest
let manifest; // TODO: Remove if unused
try {
  manifest = JSON.parse(fs.readFileSync(_MANIFEST_PATH, 'utf8'));
  log('✓ Manifest loaded successfully', 'green');
} catch (_error) {
  log('✗ Failed to load manifest: ' + error._message, 'red');
  process.exit(1);
}
let _totalTests = 0;
let _passedTests = 0;
function test(_description, condition) {
  totalTests++;
  if (condition) {
...
for (const fileName of sampleFiles) {
  const _filePath = path.join(_TEMPLATE_DIR, fileName);
...
const _sparcCommands = manifest.files.filter(f => f.category === 'sparc-mode');
for (const cmd of sparcCommands.slice(_0, 3)) { // Test first 3 commands
  const _filePath = path.join(_TEMPLATE_DIR, cmd.destination);
  if (fs.existsSync(filePath)) {
    const _content = fs.readFileSync(_filePath, 'utf8');
...
const _testFiles = manifest.files.filter(f => f.category === 'test');
for (const testFile of testFiles.slice(_0, 3)) { // Test first 3 test files
  const _filePath = path.join(_TEMPLATE_DIR, testFile.destination);
  if (fs.existsSync(filePath)) {
    const _content = fs.readFileSync(_filePath, 'utf8');
...
const _versionFile = path.join(__dirname, 'VERSION');
if (fs.existsSync(versionFile)) {
  const _fileVersion = fs.readFileSync(_versionFile, 'utf8').trim();
...
for (const [_category, info] of Object.entries(manifest.categories)) {
  const _actualCount = manifest.files.filter(f => f.category === category).length;
...
const _installFile = path.join(__dirname, '.installed');
