...
export class ProcessUI {
  ...
  private setupEventListeners(): void {
    this.processManager.on('statusChanged', ({ _processId, status }: { processId: string; status: ProcessStatus }) => {
      if (this.running) {
        this.render();
      }
    });
    this.processManager.on('processError', ({ _processId, error }: { processId: string; error: Error }) => {
      if (this.running) {
...
this.showHelp();
  ...
  private render(): void {
    ...
    processes.forEach((_process, index) => {
  ...
  private async showProcessMenu(process: ProcessInfo): Promise<void> {
    ...
    const _action = decoder.decode(buf.subarray(_0, n)).trim().toLowerCase();
...
this.showProcessDetails(process);
  ...
  private async handleExit(): Promise<void> {
    const _processes = this.processManager.getAllProcesses();
    const _hasRunning = processes.some(p => p.status === ProcessStatus.RUNNING);
    ...
    if (hasRunning) {
      ...
      if (n && decoder.decode(buf.subarray(_0, n)).trim().toLowerCase() === 'y') {
        await this.stopAll();
      }
    }
