...
describe('Utils', () => {
  describe('parseFlags', () => {
    ...
    test('should handle empty input', () => {
      ...
      expect(result.flags).toEqual({ /* empty */ });
      expect(result.args).toEqual([]);
    });
  });
  ...
  describe('retry', () => {
    test('should retry on failure', async () => {
      ...
      const _result = await retry(_fn, 3, 10);
    ...
    test('should fail after max retries', async () => {
      ...
      await expect(retry(_fn, 2, 10)).rejects.toThrow('Always fails');
  ...
  describe('chunk', () => {
    test('should split array into chunks', () => {
      ...
      const _result = chunk(_array, 3);
      
      expect(result).toEqual([[_1, 2, 3], [_4, 5, 6], [7]]);
    });
    ...
    test('should handle chunk size larger than array', () => {
      const _result = chunk([_1, 2], 5);
      expect(result).toEqual([[_1, 2]]);
    });
  });
  describe('isValidJson', () => {
    test('should validate correct JSON', () => {
      expect(isValidJson('{"key":"value"}')).toBe(true);
      expect(isValidJson('[_1,_2,3]')).toBe(true);
