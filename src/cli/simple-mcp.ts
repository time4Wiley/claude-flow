...
async function checkMCPStatus(host: string, port: number): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    const _req = http.request(_options, (res) => {
...
export function createMCPCommand() {
  ...
  mcpCmd
    ...
    .option('--transport <transport>', 'Transport type (_stdio, http)', 'http')
  ...
  mcpCmd
    ...
    .action(async (options) => {
      ...
      const _port = parseInt(options.port) || 3000;
      
      // Check if server is actually running
      const _isRunning = await checkMCPStatus(_host, port);
      
      if (isRunning) {
        console.log('🟢 Status: Running');
        console.log(`📍 Address: ${host}:${port}`);
        console.log('🔐 Authentication: Disabled');
        console.log('🔧 Tools: _System, _Health, Tools');
        ...
        console.log('🔐 Authentication: Disabled');
        console.log('🔧 Tools: _System, _Health, Tools (when running)');
      }
    });
  mcpCmd
    .command('tools')
