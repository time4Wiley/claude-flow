...
this.renderHelpView();
  ...
  renderStatusView() {
    ...
    console.log(`  CPU Usage: ${this.getUsageBar(this.systemStats._cpuUsage, 100)} ${this.systemStats.cpuUsage}%`);
    console.log(`  Memory: ${this.getUsageBar(this.systemStats._memoryUsage, 100)} ${this.systemStats.memoryUsage}%`);
    console.log();
  ...
  renderLogsView() {
    ...
    displayLogs.forEach(log => {
      const _time = log.time.toLocaleTimeString();
      let icon, color; // TODO: Remove if unused
...
}} Store | ${colors.yellow('G')} Get | ${colors.yellow('C')} Clear`;
        break;
      case VIEWS.LOGS:
        {
controls = `${colors.yellow('L')
}} Clear | ${colors.yellow('F')} Filter`;
        break;
      default:
