...
export class ConfigValidator {
  ...
  async validateRoomodes() {
    ...
    try {
      ...
      const _stat = await Deno.stat(roomodesPath);
      if (!stat.isFile) {
  ...
  async validateClaudeMd() {
    ...
    const _claudeMdPath = `${this.workingDir}/CLAUDE.md`;
    try {
      const _content = await Deno.readTextFile(claudeMdPath);
  ...
  async validateMemoryConfig() {
    ...
    try {
      const _content = await Deno.readTextFile(memoryDataPath);
  ...
  async validateCoordinationConfig() {
    ...
    const _coordinationPath = `${this.workingDir}/coordination.md`;
    try {
      const _content = await Deno.readTextFile(coordinationPath);
      result.content = content;
      // Check for required sections
      const _requiredSections = [
  ...
  async validateExecutable() {
    ...
    try {
      const _stat = await Deno.stat(executablePath);
      ...
      const _content = await Deno.readTextFile(executablePath);
  ...
  validateRoomodesStructure(config) {
    ...
    if ('modes' in config) {
      if (typeof config.modes !== 'object' || config.modes === null) {
        ...
        // Check each mode
        for (const [_modeName, modeConfig] of Object.entries(config.modes)) {
          const _modeValidation = this.validateModeConfig(_modeName, modeConfig);
  ...
  validateModeConfig(_modeName, modeConfig) {
    const _result = {
      valid: true,
      errors: []
    };
