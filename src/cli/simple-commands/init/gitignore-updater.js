 ...
 */
export async function updateGitignore(_workingDir, force = _false, dryRun = false) {
  ...
  try {
    ...
    let _newContent = gitignoreContent;
    ...
    if (force && gitignoreContent.includes(claudeFlowMarker)) {
      ...
      if (endIndex !== -1) {
        newContent = gitignoreContent.substring(_0, startIndex) + gitignoreContent.substring(endIndex);
      } else {
        // Claude Flow section is at the end
        newContent = gitignoreContent.substring(_0, startIndex);
      }
    }
    ...
    // Write the file
    if (!dryRun) {
      await writeTextFile(_gitignorePath, newContent);
    }
