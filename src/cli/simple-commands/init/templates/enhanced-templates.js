...
export function createWrapperScript(type = 'unix') {
  ...
  if (type === 'unix') {
    const _universalTemplate = loadTemplate('claude-flow-universal');
    if (universalTemplate) {
      return universalTemplate;
    }
  }
  
  const _filename = type === 'unix' ? 'claude-flow' : 
...
export function createCommandDoc(_category, command) {
  const _template = loadTemplate(`commands/${category}/${command}.md`);
  if (!template) {
    // Silently fall back to generated documentation
    return createCommandDocFallback(_category, command);
  }
  return template;
}
// Generate command documentation fallbacks
function createCommandDocFallback(_category, command) {
  const _docs = {
    analysis: {
...
```
## Options
- `--period <time>` - Analysis period (_1h, 24h, 7d, 30d)
...
```
## Options
- `--format <type>` - Report format (_json, _html, markdown)
...
```
## Options
- `--topology <type>` - Swarm topology (_mesh, _hierarchical, _ring, star)
- `--max-agents <n>` - Maximum number of agents
- `--strategy <type>` - Execution strategy (_parallel, _sequential, adaptive)
## Examples
```bash
...
```
## Options
- `--type <type>` - Agent type (_coder, _researcher, _analyst, _tester, coordinator)
...
- `--priority <level>` - Task priority (_low, _medium, _high, critical)
## Examples
```bash
...
- `--focus <area>` - Focus area (_maintenance, _features, security)
## Examples
```bash
# Create GitHub swarm
...
- `--include <areas>` - Include specific areas (_issues, _prs, _code, commits)
## Examples
```bash
# Basic analysis
...
- `--focus <areas>` - Review focus (_security, _performance, style)
...
```
## Options
- `--action <type>` - Action (_store, _retrieve, _list, clear)
- `--key <key>` - Memory key
...
```
## Options
- `--action <type>` - Action (_view, _clear, optimize)
...
export const _COMMAND_STRUCTURE = {
...
(async () => {
  ...
  const _strategies = [
    ...
    async () => {
      try {
        const _localPath = resolve(process.cwd(), 'node_modules/.bin/claude-flow');
        const { existsSync } = await import('fs');
        if (existsSync(localPath)) {
          return spawn(_localPath, process.argv.slice(2), { stdio: 'inherit' });
        }
      } catch { /* empty */ }
    },
    ...
    async () => {
      try {
        const _parentPath = resolve(process.cwd(), '../node_modules/.bin/claude-flow');
        const { existsSync } = await import('fs');
        if (existsSync(parentPath)) {
          return spawn(_parentPath, process.argv.slice(2), { stdio: 'inherit' });
        }
      } catch { /* empty */ }
    },
...
### Claude Code Handles:
- ✅ **ALL file operations** (_Read, _Write, _Edit, MultiEdit)
      ...
      PreToolUse: [
        {
          ...
          hooks: [
            {
              type: 'command',
              command: "cat | jq -r '.tool_input.command // \"\"' | xargs -I { /* empty */ } npx claude-flow@alpha hooks pre-command --command \"{ /* empty */ }\" --validate-safety true --prepare-resources true"
            }
          ]
        },
        {
          ...
          hooks: [
            {
              type: 'command',
              command: "cat | jq -r '.tool_input.file_path // .tool_input.path // \"\"' | xargs -I { /* empty */ } npx claude-flow@alpha hooks pre-edit --file \"{ /* empty */ }\" --auto-assign-agents true --load-context true"
            }
          ]
        }
      ],
      PostToolUse: [
        {
          ...
          hooks: [
            {
              type: 'command',
              command: "cat | jq -r '.tool_input.command // \"\"' | xargs -I { /* empty */ } npx claude-flow@alpha hooks post-command --command \"{ /* empty */ }\" --track-metrics true --store-results true"
            }
          ]
        },
        {
          ...
          hooks: [
            {
              type: 'command',
              command: "cat | jq -r '.tool_input.file_path // .tool_input.path // \"\"' | xargs -I { /* empty */ } npx claude-flow@alpha hooks post-edit --file \"{ /* empty */ }\" --format true --update-memory true --train-neural true"
            }
          ]
        }
      ],
