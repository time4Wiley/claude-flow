...
export async function createLocalExecutable(_workingDir, dryRun = false) {
  try {
    if (platform() === 'win32') {
...
`;
      ...
      if (!dryRun) {
        await writeFile(`${workingDir}/claude-flow.cmd`, _wrapperScript, 'utf8');
      ...
      const _isDevelopment = workingDir.includes('claude-code-flow');
...
`;
      ...
      if (!dryRun) {
        await writeFile(`${workingDir}/claude-flow`, _wrapperScript, 'utf8');
