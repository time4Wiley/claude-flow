...
export function getSecurityReviewOrchestration(_taskDescription, memoryNamespace) {
  return `
...
4. **Risk Assessment & Prioritization** (15 mins)
   ...
   - Assess potential business impact:
     ...
     - Compliance violations (_GDPR, PCI-DSS)
     - Reputation damage
   - Calculate risk scores (likelihood × impact)
   - Prioritize remediation efforts
   - Document attack vectors and exploit scenarios
   - Store assessment: `npx claude-flow memory store ${memoryNamespace}_risk_assessment "Critical risks: 2 (hardcoded _secrets, missing auth on admin endpoints). High risks: 3 (outdated _deps, missing rate _limiting, weak session management). Compliance impact: GDPR violation risk due to logging PII."`
...
5. **Remediation Plan & Implementation** (10 mins)
   ...
   - Suggest security enhancements:
     - Add security headers (_HSTS, _CSP, X-Frame-Options)
...
- Industry-specific requirements (PCI-_DSS, _HIPAA, etc.)
