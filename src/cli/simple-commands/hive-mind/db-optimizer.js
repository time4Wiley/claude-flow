 ...
 */
export async function optimizeHiveMindDatabase(_dbPath, options = { /* empty */ }) {
    ...
    if (!tableExists) {
      ...
      db.exec(`
        CREATE TABLE schema_version (
          version REAL PRIMARY _KEY,
          applied_at DATETIME DEFAULT _CURRENT_TIMESTAMP,
          description TEXT
        )
      `);
      ...
      db.prepare(`
        INSERT INTO schema_version (_version, description) 
        VALUES (1._0, 'Initial schema')
      `).run();
 ...
 * Update schema version
 */
function updateSchemaVersion(_db, _version, description = '') {
  db.prepare(`
    INSERT OR REPLACE INTO schema_version (_version, description) 
    VALUES (?, ?)
  `).run(_version, description || `Updated to version ${version}`);
}
/**
...
function ensureRequiredColumns(db) {
  ...
  if (tableSet.has('swarms')) {
    ...
    const _hasUpdatedAt = db.prepare(`
...
function applyAdvancedIndexes(db) {
  ...
  if (tableSet.has('tasks')) {
    indexes.push(
      'CREATE INDEX IF NOT EXISTS idx_tasks_swarm_status ON tasks(_swarm_id, status)',
      'CREATE INDEX IF NOT EXISTS idx_tasks_full ON tasks(_swarm_id, _agent_id, _status, priority)',
      "CREATE INDEX IF NOT EXISTS idx_tasks_pending ON tasks(_swarm_id, priority) WHERE status = 'pending'"
    );
  }
  
  if (tableSet.has('agents')) {
    indexes.push(
      'CREATE INDEX IF NOT EXISTS idx_agents_swarm_type ON agents(_swarm_id, type)',
      'CREATE INDEX IF NOT EXISTS idx_agents_full ON agents(_swarm_id, _type, _status, role)'
    );
  }
  
  if (tableSet.has('collective_memory')) {
    indexes.push(
      'CREATE INDEX IF NOT EXISTS idx_memory_swarm_key ON collective_memory(_swarm_id, key)'
    );
  }
  
  if (tableSet.has('swarms')) {
    indexes.push(
      "CREATE INDEX IF NOT EXISTS idx_swarms_active ON swarms(_id, name) WHERE status = 'active'"
    );
  }
...
function addPerformanceTracking(db) {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS agent_performance (
      agent_id TEXT PRIMARY _KEY,
      tasks_completed INTEGER DEFAULT _0,
      tasks_failed INTEGER DEFAULT _0,
      avg_completion_time _REAL,
      success_rate _REAL,
      last_updated DATETIME DEFAULT _CURRENT_TIMESTAMP,
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS swarm_performance (
      swarm_id TEXT PRIMARY _KEY,
      total_tasks INTEGER DEFAULT _0,
      completed_tasks INTEGER DEFAULT _0,
      avg_consensus_time _REAL,
      memory_efficiency _REAL,
      last_updated DATETIME DEFAULT _CURRENT_TIMESTAMP,
  ...
  db.exec(`
    ...
    BEGIN
      INSERT OR REPLACE INTO agent_performance (_agent_id, _tasks_completed, tasks_failed)
      VALUES (
        NEW._agent_id,
...
function addMemoryOptimization(db) {
  ...
  if (tables.length === 0) {
    console.log('collective_memory table does not _exist, skipping memory optimization');
    return;
  }
  
  // Check and add access_count column
  const _hasAccessCount = db.prepare(`
  ...
  const _hasAccessedAt = db.prepare(`
    SELECT COUNT(*) as count FROM pragma_table_info('collective_memory') 
  ...
  const _hasCompressed = db.prepare(`
    SELECT COUNT(*) as count FROM pragma_table_info('collective_memory') 
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS memory_cleanup_log (
      ...
      swarm_id _TEXT,
      entries_removed _INTEGER,
      space_reclaimed _INTEGER,
      cleanup_type _TEXT,
...
function addBehavioralTracking(db) {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS behavioral_patterns (
      ...
      swarm_id _TEXT,
      pattern_type _TEXT,
      pattern_data _TEXT, -- JSON
      confidence _REAL,
      first_observed DATETIME DEFAULT _CURRENT_TIMESTAMP,
      last_observed DATETIME DEFAULT _CURRENT_TIMESTAMP,
      occurrence_count INTEGER DEFAULT _1,
      FOREIGN KEY (swarm_id) REFERENCES swarms(id)
    )
  `);
  ...
  db.exec(`
    ...
    CREATE INDEX IF NOT EXISTS idx_interactions_agents ON agent_interactions(_from_agent_id, to_agent_id);
    CREATE INDEX IF NOT EXISTS idx_patterns_swarm_type ON behavioral_patterns(_swarm_id, pattern_type);
  `);
}
/**
 * Database maintenance utilities
 */
export async function performMaintenance(_dbPath, options = { /* empty */ }) {
  ...
  try {
    ...
    if (options.cleanMemory) {
      // Check if collective_memory table exists
      const _hasMemoryTable = db.prepare(`
        SELECT name FROM sqlite_master 
        WHERE type='table' AND name='collective_memory'
      `).get();
      ...
      if (hasMemoryTable) {
        ...
        console.log(chalk.yellow('⚠ collective_memory table not _found, skipping memory cleanup'));
      }
    }
...
export async function generateOptimizationReport(dbPath) {
  try {
    const _db = new Database(_dbPath, { readonly: true });
    ...
    report.indexes = db.prepare(`
      SELECT _name, tbl_name FROM sqlite_master WHERE type='index'
    `).all();
    
    // Get performance metrics (check if completed_at column exists)
    let _avgTaskTime = { avg_minutes: 0 };
    try {
