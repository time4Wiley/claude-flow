...
DEFAULT BEHAVIOR:
  ...
  including memory _coordination, agent _management, and task orchestration.
...
export async function swarmCommand(_args, flags) {
  ...
  const _outputFormat = flags && flags['output-format'];
  const _outputFile = flags && flags['output-file'];
  const _isJsonOutput = outputFormat === 'json';
  const _isNonInteractive = isJsonOutput || (flags && flags['no-interactive']);
  
  // Handle analysis/read-only mode
  const _isAnalysisMode = flags && (flags.analysis || flags['read-only']);
  const _analysisMode = isAnalysisMode ? 'analysis' : 'standard';
  ...
  if (isJsonOutput && !(flags && flags.executor)) {
    flags = { ...(flags || { /* empty */ }), executor: true };
  }
  ...
  if (flags && flags.executor) {
    ...
    try {
      ...
      const _strategy = flags.strategy || 'auto';
      const _mode = flags.mode || 'centralized';
      const _maxAgents = flags['max-agents'] || 5;
      
      // Get strategy-specific guidance
      const _strategyGuidance = getStrategyGuidance(_strategy, objective);
      const _modeGuidance = getModeGuidance(mode);
      const _agentRecommendations = getAgentRecommendations(_strategy, _maxAgents, objective);
      
      const _enableSparc = flags.sparc !== false && (strategy === 'development' || strategy === 'auto');
      
      const _swarmPrompt = `You are orchestrating a Claude Flow Swarm with advanced MCP tool coordination.
🎯 OBJECTIVE: ${objective}
🐝 SWARM CONFIGURATION:
- Strategy: ${strategy}
- Mode: ${mode}
...
2. ✅ SEARCH codebases (_Glob, Grep tools)
...
The swarm should be self-documenting - use memory_store to save all important information, decisions, and results throughout the execution.`;
      ...
      console.log('   • Coordination patterns for', _mode, 'mode');
  ...
  if (flags && flags.background && !process.env.CLAUDE_SWARM_NO_BG) {
    ...
    if (typeof Deno !== 'undefined') {
      ...
      const _objective = (args || []).join(' ').trim();
      const _swarmId = `swarm_${Date.now()}_${Math.random().toString(36).substr(_2, 9)}`;
      const _swarmRunDir = `./swarm-runs/${swarmId}`;
      
      // Create swarm directory
      await mkdirAsync(_swarmRunDir, { recursive: true });
      ...
      const _commandArgs = ['run', '--allow-all', import.meta.url, objective];
      const _newFlags = { ...flags };
      delete newFlags.background; // Remove background flag
      
      for (const [_key, value] of Object.entries(newFlags)) {
      ...
      // Create log file
      const _logFile = `${swarmRunDir}/swarm.log`;
      const _logHandle = await open(_logFile, 'w');
      
      // Create a script to run the swarm without background flag
      const _scriptContent = `#!/usr/bin/env -S deno run --allow-all
...
const _flags = ${JSON.stringify(newFlags)};
const _args = ${JSON.stringify(args)};
// Set env to prevent background spawning
process.env.CLAUDE_SWARM_NO_BG = 'true';
// Run the swarm
await swarmCommand(_args, flags);
`;
      
      const _scriptPath = `${swarmRunDir}/run-swarm.js`;
      await writeTextFile(_scriptPath, scriptContent);
      ...
      const _binDir = new URL('../../../bin/', import.meta.url).pathname;
      const _bgScriptPath = `${binDir}claude-flow-swarm-bg`;
      ...
      try {
        ...
        const _bgProcess = spawn(_bgScriptPath, _bgArgs, {
          stdio: ['ignore', 'pipe', 'pipe']
        });
        
        // Read and display output
        const _decoder = new TextDecoder();
        const _output = await bgProcess.output();
        ...
        console.log('\n⚠️  Background script not _found, using fallback method');
        
        // Create a shell script that will run the swarm
        const _shellScript = `#!/bin/bash
...
`;
        ...
        const _shellScriptPath = `${swarmRunDir}/launch-background.sh`;
        await writeTextFile(_shellScriptPath, shellScript);
        chmodSync(_shellScriptPath, 0o755);
        
        // Execute the shell script
        const _shellProcess = spawn('bash', [shellScriptPath], {
          stdio: 'ignore',
          detached: true
        });
        shellProcess.unref();
    ...
    const _bgScriptPath = path.join(path.dirname(new URL(import.meta.url).pathname), '../../../bin/claude-flow-swarm-bg');
    
    // Check if background script exists
    if (fs.existsSync(bgScriptPath)) {
      // Build command args
      const _commandArgs = [objective];
      ...
      console.log('\nFor true background _execution, use:');
      console.log(`  nohup claude-flow swarm "${objective}" ${Object.entries(flags).filter(([_k,v]) => k !== 'background' && v).map(([_k,v]) => `--${k}${v !== true ? ` ${v}` : ''}`).join(' ')} > swarm.log 2>&1 &`);
    }
    return;
  }
  ...
  try {
    ...
    let swarmAction; // TODO: Remove if unused
    try {
      // Try the compiled version first (for production/npm packages)
      const _distPath = new URL('../../../dist/cli/commands/swarm-new.js', import.meta.url);
      const _module = await import(distPath);
      swarmAction = module.swarmAction;
    } catch (distError) {
      // If dist version not found, try TypeScript version (for development)
      try {
        const _module = await import('../commands/swarm-new.ts');
        swarmAction = module.swarmAction;
      } catch (tsError) {
    ...
    if (error.code === 'ERR_MODULE_NOT_FOUND' || error.code === 'ERR_UNSUPPORTED_NODE_MODULES_TYPE_STRIPPING' || error.code === 'ERR_UNKNOWN_FILE_EXTENSION') {
      ...
      try {
        ...
        const _result = await executeSwarm(_objective, flags);
        ...
        try {
          await createSwarmFiles(_objective, flags);
          return;
        } catch (createError) {
      ...
      try {
        ...
        if (swarmDemoPath && Deno) {
          ...
          for (const [_key, value] of Object.entries(flags || { /* empty */ })) {
            swarmArgs.push(`--${key}`);
            if (value !== true) {
          ...
          const _swarmProcess = spawn('node', [_swarmDemoPath, ...swarmArgs], {
            stdio: 'inherit'
          });
      ...
      try {
        ...
        const _enableSparc = flags.sparc !== false;
        const _swarmPrompt = `Execute a swarm coordination task using ${enableSparc ? 'the full SPARC methodology' : 'standard approach'}:
OBJECTIVE: ${objective}
CONFIGURATION:
...
5. Include all necessary files (source _code, _tests, _configs, documentation)
...
- Include configuration files (package._json, requirements._txt, etc.)
...
async function createSwarmFiles(_objective, flags) {
  const _fs = await import('fs');
  const _path = await import('path');
  ...
  const _targetMatch = objective.match(/ins+([^s]+)/?$/i);
  let _targetDir = targetMatch ? targetMatch[1] : 'output';
  ...
  await fs.promises.mkdir(_targetDir, { recursive: true });
  
  // Determine what to build based on objective
  const _isRestAPI = objective.toLowerCase().includes('rest api') || 
                    objective.toLowerCase().includes('api');
  
  if (isRestAPI) {
    // Create REST API
    const _apiDir = path.join(_targetDir, 'rest-api');
    await fs.promises.mkdir(_apiDir, { recursive: true });
...
`;
    ...
    await fs.promises.writeFile(path.join(_apiDir, 'server.js'), serverCode);
    ...
    await fs.promises.writeFile(
      path.join(_apiDir, 'package.json'), 
...
`;
    ...
    await fs.promises.writeFile(path.join(_apiDir, 'README.md'), readme);
    ...
    const _appCode = `// Application created by Claude Flow Swarm
// Objective: ${objective}
// Swarm ID: ${swarmId}
function main() {
...
`;
    ...
    await fs.promises.writeFile(path.join(_targetDir, 'app.js'), appCode);
    ...
    await fs.promises.writeFile(
      path.join(_targetDir, 'package.json'), 
      JSON.stringify(_packageJson, null, 2)
    );
    console.log('  ✅ Created: package.json');
 ...
 */
function getStrategyGuidance(_strategy, objective) {
  const _guidanceMap = {
...
COORDINATION PATTERN:
- Spawn domain-specific coordinators (e.g., frontend-_lead, backend-lead)
 ...
 */
function getAgentRecommendations(_strategy, _maxAgents, objective) {
