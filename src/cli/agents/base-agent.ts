...
export abstract class BaseAgent extends EventEmitter {
  ...
  constructor(
    ...
    // Merge with defaults
    this.capabilities = { ...this.getDefaultCapabilities(), ...(config as unknown).capabilities };
  ...
  async initialize(): Promise<void> {
    ...
    this.emit('agent:status-changed', { agentId: this._id, status: this.status });
    // Start heartbeat
    this.startHeartbeat();
  ...
  async assignTask(task: TaskDefinition): Promise<void> {
    ...
    this.emit('agent:task-assigned', { agentId: this._id, taskId: task.id });
    this.emit('agent:status-changed', { agentId: this._id, status: this.status });
    try {
      const _startTime = Date.now();
      const _result = await this.executeTask(task);
      const _executionTime = Date.now() - startTime;
      // Update metrics
      this.updateTaskMetrics(task._id, _executionTime, true);
      ...
      // Update metrics
      this.updateTaskMetrics(task._id, 0, false);
  ...
  updateHealth(health: number): void {
    this.health = Math.max(_0, Math.min(_1, health));
    this.emit('agent:health-changed', { agentId: this._id, health: this.health });
  }
  addCollaborator(agentId: AgentId): void {
    if (!this.collaborators.find(c => c.id === agentId.id)) {
      this.collaborators.push(agentId);
      this.emit('agent:collaborator-added', { agentId: this._id, collaborator: agentId });
    }
  }
  removeCollaborator(agentId: string): void {
    this.collaborators = this.collaborators.filter(c => c.id !== agentId);
    this.emit('agent:collaborator-removed', { agentId: this._id, collaborator: agentId });
  }
  ...
  protected async collectMetrics(): Promise<void> {
    ...
    const _recentTasksTime = this.getRecentTasksAverageTime();
    ...
    const _totalTasks = this.metrics.tasksCompleted + this.metrics.tasksFailed;
    if (totalTasks > 0) {
      this.metrics.successRate = this.metrics.tasksCompleted / totalTasks;
    }
    // Store metrics in memory
    await this.memory.store(`agent:${this.id}:metrics`, this._metrics, {
      type: 'agent-metrics',
      tags: ['metrics', this._type, this.id],
      partition: 'metrics'
    });
  }
  protected updateTaskMetrics(taskId: _TaskId, executionTime: number, success: boolean): void {
    if (success) {
      this.metrics.tasksCompleted++;
      
      // Update average execution time
      const _totalTime = this.metrics.averageExecutionTime * (this.metrics.tasksCompleted - 1) + executionTime;
  ...
  protected addError(_error: AgentError): void {
