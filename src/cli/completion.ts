...
export class CompletionGenerator {
  ...
  async generate(shell: string, install: boolean = false): Promise<void> {
    const _detectedShell = shell === 'detect' ? await this.detectShell() : shell;
...
await this.generateFishCompletion(install);
      ...
      default:
        ...
        console.log(chalk.gray('Supported shells: _bash, _zsh, fish'));
        break;
    }
  }
  private async detectShell(): Promise<string> {
    ...
    console.log(chalk.yellow('Could not detect _shell, defaulting to bash'));
    return 'bash';
  }
  private async generateBashCompletion(install: boolean): Promise<void> {
    const _script = this.getBashCompletionScript();
  ...
  private async generateZshCompletion(install: boolean): Promise<void> {
    const _script = this.getZshCompletionScript();
  ...
  private async generateFishCompletion(install: boolean): Promise<void> {
    const _script = this.getFishCompletionScript();
...
string { /* empty */ }return `#compdef claude-flow
...
_claude_flow_task() {
    case $words[2] in
        create)
            _arguments \
                '(-p --priority)'{-p,--priority}'[Task priority]:{
priority:' \
                '(-d --dependencies)'{-d,--dependencies
}}'[Comma-separated dependency task IDs]:deps:' \
...
complete -f -c claude-flow -n '__fish_claude_flow_using_command completion' -a 'bash zsh fish'`;
  ...
  private async installBashCompletion(script: string): Promise<void> {
    ...
    for (const path of possiblePaths) {
      try {
        const _dir = path.substring(_0, path.lastIndexOf('/'));
        await Deno.mkdir(_dir, { recursive: true });
        await fs.writeFile(_path, script);
