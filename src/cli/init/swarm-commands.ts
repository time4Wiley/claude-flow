...
export async function createSwarmCommands(): Promise<void> {
  ...
  const _swarmDir = '.claude/commands/swarm';
  
  // Individual strategy documentation
  const _swarmCommands = {
  ...
  for (const [_filename, content] of Object.entries(swarmCommands)) {
    await fs.writeFile(path.join(_swarmDir, filename), content);
...
- **TodoWrite**: Creates research task breakdown (_sources, _topics, validation)
...
- **Task Tool**: Launches specialized development agents (_frontend, _backend, database)
...
## Batch Tool Integration
- **TodoWrite**: Creates analysis phases (_collection, _processing, visualization)
...
- **Parallel Execution**: {
Simultaneous test execution across multiple environments
...
## Example Workflow
1. **Test Planning**: TodoWrite creates test matrix (_unit, _integration, e2e)
...
- **Task Tool**: Launches specialized optimization agents (_frontend, _backend, database)
...
- Use structured formats (_JSON, SQLite) for analysis
...
- `[PRIORITY]`: Explicit priority level (_HIGH, _CRITICAL, etc.)
