...
export async function createDirectoryStructure(): Promise<void> {
  ...
  for (const [_filePath, content] of Object.entries(readmeFiles)) {
  ...
  await fs.writeFile('memory/claude-flow-data.json', JSON.stringify(_initialData, null, 2));
...
- Multiple output formats supported (_JSON, _SQLite, _CSV, HTML)
## Usage
