...
`;
            ...
            try {
              ...
              const _command = new Deno.Command('claude', {
                args: _claudeArgs,
                env: {
                  ...Deno.env.toObject(),
...
`);
    ...
    config: async (key: string) => {
      try {
        const _config = JSON.parse(await fs.readFile('claude-flow.config.json'));
        if (key) {
  ...
  async function handleAgentCommand(args: string[], state: unknown) {
...
if (state.context.agents.length === 0) {
          ...
          state.context.agents.forEach((agent: unknown) => {
      ...
      case 'info': {
        ...
        const _foundAgent = state.context.agents.find((a: unknown) => a.id === agentId || a.name === agentId);
        if (foundAgent) {
      ...
      case 'terminate': {
        ...
        const _index = state.context.agents.findIndex((a: unknown) => a.id === termId || a.name === termId);
        if (index >= 0) {
  ...
  async function handleTaskCommand(args: string[], state: unknown) {
    const _subCmd = args[0];
    switch (subCmd) {
      case 'create': {
...
if (state.context.tasks.length === 0) {
          ...
          state.context.tasks.forEach((task: unknown) => {
      ...
      case 'assign': {
        ...
        const _foundTask = state.context.tasks.find((t: unknown) => t.id === taskId);
        const _assignAgent = state.context.agents.find((a: unknown) => a.id === assignAgentId || a.name === assignAgentId);
      ...
      case 'status': {
        ...
        const _statusTask = state.context.tasks.find((t: unknown) => t.id === statusId);
        if (statusTask) {
  ...
  async function handleMemoryCommand(args: string[], state: unknown) {
    const _subCmd = args[0];
    switch (subCmd) {
      case 'store': {
        const _key = args[1];
...
state.context.memory = { /* empty */ }};
  ...
  async function handleTerminalCommand(args: string[], state: unknown) {
    const _subCmd = args[0];
    switch (subCmd) {
      case 'create': {
...
if (state.context.terminals.length === 0) {
          ...
          state.context.terminals.forEach((term: unknown) => {
...
if (state.currentSession) {
  ...
  while (true) {
    // Show prompt
    const _prompt = replState.currentSession ? 
    ...
    const _input = decoder.decode(buf.subarray(_0, n)).trim();
