...
import { Agent, Tool, Workflow, Mastra, generateEmptyFromSchema, Telemetry } from '@mastra/core';
...
import { isVercelTool, Tool as Tool$1 } from '@mastra/core/tools';
...
class WorkflowEngine extends EventEmitter$1 {
  ...
  async executeAgentTask(execution, step) {
    const { StepExecutorFactory } = await import('./step-executors.mjs');
...
class AgentManager extends EventEmitter$1 {
  ...
  async createAgent(config) {
    ...
    logger$1.info("Agent created", { agentId: agent.id, name: agent.name });
    return agent;
  }
  async startAgent(agentId) {
    ...
    logger$1.info("Agent started", { agentId, name: agent.name });
  ...
  async stopAgent(agentId, force = false) {
    ...
    logger$1.info("Agent stopped", { agentId, name: agent.name, force });
  }
  async removeAgent(agentId) {
    ...
    logger$1.info("Agent removed", { agentId, name: agent.name });
  }
  getAgent(agentId) {
  ...
  async assignTask(agentId, task) {
    ...
    logger$1.info("Task assigned to agent", { agentId, task });
    ...
    try {
      ...
      logger$1.info("Task completed", { agentId, duration: responseTime });
      return result;
    } catch (error) {
      ...
      logger$1.error("Task failed", { agentId, error: error.message });
      throw error;
    }
  }
...
export { CoordinatorAgent as C, ExecutorAgent as E };
