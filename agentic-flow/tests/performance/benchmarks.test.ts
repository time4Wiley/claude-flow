...
import { createMockProvider, generateTestData } from '../utils/test-helpers';
import { MockAnthropicClient } from '../utils/mock-providers';

// Mock providers for consistent benchmarking
jest.mock('@anthropic-ai/sdk', () => ({
  __esModule: true,
  default: jest.fn().mockImplementation(() => new MockAnthropicClient()),
}));
...
describe('Performance Benchmarks', () => {
    ...
    it('should benchmark provider switching overhead', async () => {
      ...
      for (let i = 0; i < 5; i++) {
        manager.registerProvider(
          `provider-${i}`,
          createMockProvider({ name: `provider-${i}` })
        );
      }
  ...
  describe('Message Processing Performance', () => {
    it('should benchmark message batching performance', async () => {
      ...
      for (const batchSize of batchSizes) {
        const messages = generateTestData.messages(batchSize);
    ...
    it('should benchmark concurrent request handling', async () => {
      ...
      for (const concurrency of concurrencyLevels) {
        ...
        const promises = Array.from({ length: concurrency }, () =>
          provider.generateResponse([{ role: 'user', content: 'Concurrent test' }])
        );
  ...
  describe('Memory Performance', () => {
    it('should benchmark memory store operations', async () => {
      const memoryStore = {
        ...
        get: jest.fn(async (key: string) => memoryStore.data.get(key)),
        set: jest.fn(async (key: string, value: any) => {
          memoryStore.data.set(key, value);
          return true;
        }),
        delete: jest.fn(async (key: string) => memoryStore.data.delete(key)),
    ...
    it('should benchmark memory search performance', async () => {
      ...
      for (const pattern of searchPatterns) {
        ...
        const results = Array.from(data.entries()).filter(([key, value]) => 
          JSON.stringify(value).includes(pattern)
        );
  ...
  describe('Agent Performance', () => {
    ...
    it('should benchmark multi-agent coordination', async () => {
      const agentCount = 10;
      const agents = generateTestData.agents(agentCount);
    ...
    it('should handle high load scenarios', async () => {
      ...
      for (const { requests, duration } of loadLevels) {
        ...
        while (Date.now() - startTime < duration) {
          promises.push(
            provider.generateResponse([{ role: 'user', content: 'Load test' }])
