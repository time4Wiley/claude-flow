...
const generateTimestamp = () => (/* @__PURE__ */ new Date()).toISOString();
const infrastructureState = {
  containers: /* @__PURE__ */ new Map(),
  kubernetesResources: /* @__PURE__ */ new Map(),
  pipelines: /* @__PURE__ */ new Map(),
  deployments: /* @__PURE__ */ new Map(),
  monitors: /* @__PURE__ */ new Map(),
  loadBalancers: /* @__PURE__ */ new Map(),
...
const containerManagementTool = createTool({
  ...
  execute: async ({ action, containerName, image, ports, environment, volumes }) => {
    switch (action) {
      ...
      case "start": {
        const container = Array.from(infrastructureState.containers.values()).find((c) => c.name === containerName || c.id === containerName);
        if (container) {
  ...
  execute: async ({ action, resource, name, namespace, replicas, image, config }) => {
    ...
    switch (action) {
      ...
      case "scale": {
        ...
        if (k8sResource && replicas !== void 0) {
...
const cicdPipelineManagementTool = createTool({
  ...
  execute: async ({ action, pipelineName, branch, stages, parameters }) => {
    switch (action) {
      ...
      case "trigger": {
        const pipeline = Array.from(infrastructureState.pipelines.values()).find((p) => p.name === pipelineName);
        if (pipeline) {
...
const infrastructureProvisioningTool = createTool({
  ...
  execute: async ({ action, resourceType, name, provider, specifications }) => {
    ...
    switch (action) {
      ...
      case "list": {
        const resources = Array.from(infrastructureState.deployments.values()).filter((r) => r.type === resourceType && r.provider === provider);
        return {
          success: true,
          resources,
...
const monitoringSetupTool = createTool({
  ...
  execute: async ({ action, monitorName, targetType, metrics, alertChannels }) => {
    switch (action) {
      ...
      case "update": {
        const monitor = Array.from(infrastructureState.monitors.values()).find((m) => m.name === monitorName);
        if (monitor) {
      ...
      case "alert": {
        ...
        if (monitor) {
          const recentAlerts = monitor.alerts.filter(
            (a) => new Date(a.timestamp) > new Date(Date.now() - 36e5)
            // Last hour
          );
          return {
            success: true,
...
const loadBalancingTool = createTool({
  ...
  execute: async ({ action, balancerName, algorithm, healthCheck, targets, stickySessions }) => {
    switch (action) {
      case "create": {
        ...
        setInterval(() => {
          if (loadBalancer.status === "active") {
            loadBalancer.metrics.requestsPerSecond = Math.floor(Math.random() * 1e3);
      ...
      case "update": {
        const loadBalancer = Array.from(infrastructureState.loadBalancers.values()).find((lb) => lb.name === balancerName);
        if (loadBalancer) {
          if (algorithm) loadBalancer.algorithm = algorithm;
          if (healthCheck) loadBalancer.healthCheck = healthCheck;
          if (stickySessions !== void 0) loadBalancer.stickySessions = stickySessions;
        ...
        if (drPlan) {
          ...
          const executeTestStep = () => {
            if (stepIndex < testSteps.length) {
              ...
              stepIndex++;
              setTimeout(executeTestStep, 2e3);
            } else {
              testResults.status = "completed";
              testResults.endTime = generateTimestamp();
              testResults.metrics = {
                actualRPO: rpo ? rpo - 2 : 13,
                // Slightly better than target
        ...
        if (drPlan) {
          ...
          const executeRecoveryStep = () => {
            if (execution.currentStep < execution.steps.length) {
              ...
              setTimeout(executeRecoveryStep, 3e3);
            } else {
...
export { devOpsTools };
//# sourceMappingURL=9e8350e6-b1ea-48cc-9ed0-424ad86c8463.mjs.map
