...
const statisticalAnalysis = createTool({
  ...
  inputSchema: z.object({
    ...
    metrics: z.array(z.enum(["mean", "median", "mode", "std_dev", "variance", "skewness", "kurtosis", "quantiles"])).optional().describe("Specific metrics to calculate")
  }),
  ...
  execute: async ({ datasetId, metrics = ["mean", "median", "std_dev"] }) => {
    ...
    if (!data) {
      data = generateRandomData(1e3, 0, 100);
...
const dataVisualization = createTool({
...
const correlationAnalysis = createTool({
  ...
  execute: async ({ datasetId, variables, method }) => {
    ...
    const insights = [
      ...
      `Strongest correlation: ${significantCorrelations.length > 0 ? `${significantCorrelations[0].var1} and ${significantCorrelations[0].var2} (r=${significantCorrelations[0].correlation.toFixed(3)})` : "No strong correlations found"}`,
...
const regressionAnalysis = createTool({
...
const clusteringAnalysis = createTool({
  ...
  execute: async ({ datasetId, features, algorithm, numClusters }) => {
    const k = numClusters || Math.floor(Math.random() * 3) + 3;
    ...
    const metrics = {
      ...
      // 0.4-0.8
      davies_bouldin_score: 0.5 + Math.random() * 0.5,
      // 0.5-1.0 (lower is better)
      calinski_harabasz_score: 100 + Math.random() * 200
      // 100-300
    };
    return {
      clusters,
...
const timeSeriesAnalysis = createTool({
    ...
    if (forecastPeriods) {
      ...
      forecast = {
        values: Array.from(
          ...
          (_, i) => lastValue + trendSlope * (i + 1) + seasonal[i % 12]
        ),
...
const dataQualityCheck = createTool({
  ...
  execute: async ({ datasetId, columns = ["col1", "col2", "col3"], checks }) => {
    ...
    const overallQualityScore = Object.values(columnMetrics).reduce((sum, metric) => sum + metric.validityScore, 0) / columns.length;
    const recommendations = [
...
const dataSampling = createTool({
  ...
  execute: async ({ datasetId, samplingMethod, sampleSize, stratifyColumn, randomSeed }) => {
    ...
    const z2 = 1.96;
    const marginOfError = z2 * (popStd / Math.sqrt(actualSampleSize));
    return {
      sample: {
...
const hypothesisTesting = createTool({
  ...
  execute: async ({ testType, data, hypotheses, alpha }) => {
    ...
    switch (testType) {
      case "t-test":
        ...
        effectSizeValue = Math.abs(statistic) / Math.sqrt(100);
        break;
      case "chi-square":
        statistic = Math.random() * 20;
        degreesOfFreedom = 4;
        pValue = Math.random() * 0.1;
        effectSizeValue = Math.sqrt(statistic / 100);
        break;
      case "anova":
        statistic = Math.random() * 10;
        degreesOfFreedom = 3;
        pValue = Math.random() * 0.1;
        effectSizeValue = statistic / (statistic + degreesOfFreedom);
        break;
      default:
    ...
    const effectInterpretation = effectSizeValue < 0.2 ? "negligible" : effectSizeValue < 0.5 ? "small" : effectSizeValue < 0.8 ? "medium" : "large";
    ...
    const conclusion = rejectNull ? `Reject the null hypothesis. There is sufficient evidence to support the alternative hypothesis (p = ${pValue.toFixed(4)} < \u03B1 = ${alpha}).` : `Fail to reject the null hypothesis. There is insufficient evidence to support the alternative hypothesis (p = ${pValue.toFixed(4)} > \u03B1 = ${alpha}).`;
    return {
      testResults: {
        statistic,
        pValue,
...
const reportGeneration = createTool({
...
5. Data quality assessment reveals 95% completeness across critical fields`,
    ...
    const totalWords = reportSections.reduce(
      (sum, section) => sum + section.content.split(" ").length,
      0
    );
    return {
      report: {
...
export { clusteringAnalysis, correlationAnalysis, dataAnalysisTools, dataQualityCheck, dataSampling, dataVisualization, hypothesisTesting, regressionAnalysis, reportGeneration, statisticalAnalysis, timeSeriesAnalysis };
//# sourceMappingURL=79b63929-92f2-45a6-a7b0-57d3b2c1807e.mjs.map
