...
const competitorAnalysis = createTool({
  ...
  execute: async ({ competitors }) => {
    const marketLeader = competitors.reduce(
      (prev, current) => prev.marketShare > current.marketShare ? prev : current
    ).name;
...
const userFeedbackAnalysis = createTool({
  ...
  execute: async ({ feedbackItems }) => {
    ...
    const topCategories = Object.entries(categoryMap).map(([category, data]) => ({
      category,
      count: data.count,
...
const successMetricsTracker = createTool({
  ...
  execute: async ({ metrics, period }) => {
    ...
    const overallHealth = achievedCount >= metrics.length * 0.8 ? "Excellent" : achievedCount >= metrics.length * 0.6 ? "Good" : "Needs Attention";
    return {
      report: {
        period,
        overallHealth,
...
//# sourceMappingURL=5c049f52-a0d8-42bd-b361-131a4824e360.mjs.map
