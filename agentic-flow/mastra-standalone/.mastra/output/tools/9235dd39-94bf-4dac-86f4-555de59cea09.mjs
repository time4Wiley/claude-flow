  ...
  execute: async ({ topology, maxAgents, strategy, metadata, swarmId }) => {
    ...
    if (swarmRegistry.has(id)) {
      ...
      return {
        ...
        topology,
        status: "failed",
        config: { maxAgents, strategy, createdAt: (/* @__PURE__ */ new Date()).toISOString() },
...
const agentSpawn = createTool({
  ...
  inputSchema: z.object({
    ...
    resources: z.object({
      ...
      timeout: z.number().min(1e3).max(3e5).default(6e4)
    }).optional()
  }),
  outputSchema: z.object({
    agentId: z.string(),
  ...
  execute: async ({ swarmId, agentType, name, capabilities, priority, resources }) => {
    ...
    const agent = {
      ...
      priority,
      resources: resources || { cpu: 1, memory: 1024, timeout: 6e4 },
      status: "active",
      tasks: /* @__PURE__ */ new Set(),
      metrics: {
        tasksCompleted: 0,
...
const taskOrchestrate = createTool({
  id: "task-orchestrate",
  ...
  inputSchema: z.object({
    ...
    timeout: z.number().min(1e3).max(36e5).default(3e5)
  }),
  outputSchema: z.object({
    taskId: z.string(),
    swarmId: z.string(),
...
const swarmStatus = createTool({
  ...
  execute: async ({ swarmId, includeAgents, includeTasks, includeMetrics }) => {
    ...
    const metrics = includeMetrics ? {
      uptime: `${Math.floor(uptime / 1e3 / 60)} minutes`,
      avgTaskCompletionTime: swarm.metrics.avgCompletionTime,
      successRate: swarm.metrics.tasksCompleted > 0 ? swarm.metrics.tasksCompleted / (swarm.metrics.tasksCompleted + swarm.metrics.tasksFailed) * 100 : 0,
      throughput: swarm.metrics.tasksCompleted / (uptime / 1e3 / 60)
      // tasks per minute
    } : void 0;
    let status = "active";
    ...
    if (actualScaleType === "up" && targetSize > previousSize) {
      ...
      for (let i = 0; i < agentsToAdd; i++) {
        ...
        const agent = {
          ...
          priority: "medium",
          resources: { cpu: 1, memory: 1024, timeout: 6e4 },
          status: "active",
          tasks: /* @__PURE__ */ new Set(),
          metrics: {
            tasksCompleted: 0,
      ...
      const removableCandidates = currentAgents.filter((a) => !preserveSet.has(a.id) && a.tasks.size === 0).sort((a, b) => a.metrics.utilization - b.metrics.utilization);
...
const swarmDestroy = createTool({
  ...
  inputSchema: z.object({
    ...
    timeout: z.number().min(0).max(6e4).default(5e3).describe("Graceful shutdown timeout in ms")
  }),
  outputSchema: z.object({
    swarmId: z.string(),
  ...
  execute: async ({ swarmId, force, preserveData, timeout }) => {
    ...
    const activeTasks = Array.from(swarm.tasks).map((id) => taskQueue.get(id)).filter((t) => t && t.status === "in_progress");
    if (activeTasks.length > 0 && !force) {
      ...
      return {
        ...
        finalMetrics: {
          ...
          uptime: `${Math.floor((Date.now() - new Date(swarm.createdAt).getTime()) / 1e3 / 60)} minutes`
        },
        preservedData: false,
...
const topologyOptimize = createTool({
  ...
  execute: async ({ swarmId, targetMetric, constraints }) => {
    ...
    return {
      ...
      status,
      message: status === "optimized" ? `Topology optimization can improve ${targetMetric} by ${Math.round(optimizationScore)}%` : "Current topology is already optimal for the workload"
    };
  }
});
const loadBalance = createTool({
  id: "load-balance",
  name: "Load Balance",
  ...
  execute: async ({ swarmId, tasks, strategy }) => {
    ...
    const agents = Array.from(swarm.agents).map((id) => agentRegistry.get(id)).filter((a) => a && a.status === "active");
    if (agents.length === 0) {
    ...
    for (const task of sortedTasks) {
      ...
      switch (strategy) {
        case "round-robin":
          ...
          break;
        case "least-loaded":
          selectedAgent = agents.reduce(
            (min, agent) => (agentLoads.get(agent.id) || 0) < (agentLoads.get(min.id) || 0) ? agent : min
          );
          break;
        case "capability-match":
          if (task.requiredCapabilities && task.requiredCapabilities.length > 0) {
            const capableAgents = agents.filter(
              (agent) => task.requiredCapabilities.every((cap) => agent.capabilities.includes(cap))
            );
            if (capableAgents.length > 0) {
              selectedAgent = capableAgents.reduce(
                (min, agent) => (agentLoads.get(agent.id) || 0) < (agentLoads.get(min.id) || 0) ? agent : min
              );
            }
          }
        ...
        default:
          ...
          if (task.requiredCapabilities && task.requiredCapabilities.length > 0) {
            const capableAgents = agents.filter(
              (agent) => task.requiredCapabilities.some((cap) => agent.capabilities.includes(cap))
            );
            if (capableAgents.length > 0) {
...
const coordinationSync = createTool({
...
const agentMetrics = createTool({
  ...
  execute: async ({ agentId, timeRange, includeTaskHistory, includeResourceUsage }) => {
    ...
    const uptimeMinutes = Math.floor(uptimeMs / 1e3 / 60);
    const uptime = uptimeMinutes >= 60 ? `${Math.floor(uptimeMinutes / 60)} hours ${uptimeMinutes % 60} minutes` : `${uptimeMinutes} minutes`;
    const totalTasks = agent.metrics.tasksCompleted + agent.metrics.tasksFailed;
    const successRate = totalTasks > 0 ? agent.metrics.tasksCompleted / totalTasks * 100 : 0;
    const throughput = uptimeMinutes > 0 ? agent.metrics.tasksCompleted / uptimeMinutes : 0;
    const performance = {
    ...
    const taskHistory = includeTaskHistory ? Array.from(agent.tasks).map((taskId) => {
      ...
      return {
        ...
        startTime: task?.createdAt || (/* @__PURE__ */ new Date()).toISOString(),
        endTime: task?.status === "completed" ? (/* @__PURE__ */ new Date()).toISOString() : void 0,
        duration: task?.status === "completed" ? Math.floor(Math.random() * 3e4) + 5e3 : void 0
      };
    }).slice(0, 10) : void 0;
    let health = "healthy";
...
function assignAgentsToTasks(agents, tasks, strategy) {
  ...
  if (strategy === "parallel") {
    ...
    const bestAgent = agents.reduce(
      (best, agent) => agent.metrics.successRate > best.metrics.successRate ? agent : best
    );
    tasks.forEach((task) => {
    ...
    tasks.forEach((task) => {
      const capableAgents = task.requiredCapabilities && task.requiredCapabilities.length > 0 ? agents.filter(
        (agent) => task.requiredCapabilities.some((cap) => agent.capabilities.includes(cap))
      ) : agents;
      const selectedAgent = capableAgents.length > 0 ? capableAgents.reduce(
        (least, agent) => agent.metrics.utilization < least.metrics.utilization ? agent : least
      ) : agents[0];
...
function analyzeSwarmMetrics(swarm, agents, tasks) {
  ...
  return {
    latency: 100 - avgUtilization,
    // Simplified metric
    throughput: swarm.metrics.tasksCompleted / Math.max((Date.now() - new Date(swarm.createdAt).getTime()) / 1e3 / 60, 1),
...
function calculateAgentScore(agent, task, currentLoad) {
  let score = 100 - currentLoad;
  if (task.requiredCapabilities) {
    const matchingCaps = task.requiredCapabilities.filter(
...
export { agentMetrics, agentSpawn, coordinationSync, loadBalance, swarmDestroy, swarmInit, swarmOrchestrationTools, swarmScale, swarmStatus, taskOrchestrate, topologyOptimize };
//# sourceMappingURL=9235dd39-94bf-4dac-86f4-555de59cea09.mjs.map
